CREATE DATABASE /*!32312 IF NOT EXISTS*/`travelproject` /*!40100 DEFAULT CHARACTER SET latin1 */;
USE `travelproject`;

drop table if exists trips;
CREATE TABLE  trips(tripid integer,faremethod integer,fare integer,loc_from integer,loc_to integer,
driverid integer,custid integer,distance integer,duration integer); 
...
CREATE TABLE  trips_details1(tripid	integer,loc_from integer,searches integer,searches_got_estimate integer,
searches_for_quotes integer,searches_got_quotes	integer,customer_not_cancelled	integer,
driver_not_cancelled integer,otp_entered integer,end_ride integer); 
...
CREATE TABLE  trips_details2(tripid	integer,loc_from integer,searches integer,
searches_got_estimate integer,searches_for_quotes integer,searches_got_quotes	integer,
...
CREATE TABLE  trips_details3(tripid	integer,loc_from integer,searches integer,
searches_got_estimate integer,searches_for_quotes integer,searches_got_quotes	integer,
customer_not_cancelled	integer,driver_not_cancelled integer,otp_entered integer,end_ride integer); 
...
select sum(searches_got_estimate) *100.0/sum(searches) as SGM from trips_details4;

-- Estimate to search for quote rate
select * from trips_details4;
select sum(searches_for_quotes) *100.0/sum(searches) as SFQ from trips_details4;

-- Quote acceptance rate
select sum(searches_got_quotes) *100.0/sum(searches) as SGQ from trips_details4;
...
select * from 
 ...
 -- which area got the highest cancellation trips
 -- Driver cancelled
 select * from 
 (select *, rank() over(order by can desc) rnk from
 (select loc_from,count(*) - sum(driver_not_cancelled) can from trips_details4
 group by loc_from)b)c
 where rnk=1;
 ...
 select * from 
 (select *, rank() over(order by can desc) rnk from
 (select loc_from,count(*) - sum(customer_not_cancelled) can from trips_details4
 group by loc_from)b)c
 where rnk=1;
